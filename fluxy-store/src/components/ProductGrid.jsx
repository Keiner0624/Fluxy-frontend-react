...
export default function ProductGrid({ products, loading, error, onAddToCart, onViewDetail }) {
    if (loading) return (
        <section style={{ padding: '40px 32px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 24 }}>
            {[...Array(6)].map((_, i) => (
    ...
    if (!products.length) return (
        <section style={{ padding: '80px 32px', textAlign: 'center', color: 'var(--text-muted)' }}>
