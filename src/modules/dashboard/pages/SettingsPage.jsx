...
// ─── Componente de dominio personalizado ────────────────────────────────────
function CustomDomainSection({ plan }) {
  const navigate = useNavigate()
  const [domain, setDomain]         = useState('')
  const [currentDomain, setCurrentDomain] = useState('')
  const [domainStatus, setDomainStatus]   = useState('none') // none | pending | verified | error
  ...
  const handleAddDomain = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/domains/add`, {
        method: 'POST',
  ...
  const handleRemoveDomain = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/domains/remove`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${getToken()}` },
      })
      const data = await res.json()
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${isBusiness ? 'rgba(52,211,153,0.2)' : 'rgba(255,255,255,0.06)'}`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        {!isBusiness && (
          <span style={{ fontSize: 10, fontWeight: 700, color: '#34d399', background: 'rgba(52,211,153,0.15)', border: '1px solid rgba(52,211,153,0.25)', borderRadius: 6, padding: '2px 8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            BUSINESS
          </span>
        )}
      </div>

      {/* Bloqueado para no-Business */}
      {!isBusiness ? (
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          <div style={{ fontSize: 36, marginBottom: 12 }}>🔒</div>
          <div style={{ fontSize: 14, color: 'var(--text-muted)', marginBottom: 16, lineHeight: 1.6 }}>
            Conecta tu propio dominio (ej: <strong style={{ color: 'white' }}>mitienda.com</strong>) y quita el branding de Fluxy. Exclusivo del plan Business.
          </div>
          <button onClick={() => navigate('/dashboard/plans')} style={{
            background: 'linear-gradient(135deg, #34d399, #059669)',
            color: 'white', border: 'none', borderRadius: 10,
            padding: '10px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer',
          }}>🚀 Ver plan Business</button>
        </div>
      ) : (
        <>
          {/* Dominio actual */}
          {currentDomain ? (
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>Dominio activo:</div>
              <div style={{
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.2)',
                borderRadius: 12, padding: '12px 16px', flexWrap: 'wrap', gap: 10,
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, marginTop: 2 }}>
                      ...
                      {domainStatus === 'pending'  && <span style={{ color: '#fbbf24' }}>⏳ Pendiente de verificación</span>}
                      {domainStatus === 'error'    && <span style={{ color: '#f87171' }}>❌ Error de verificación</span>}
                    </div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={loadDomainStatus} style={{ padding: '6px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-soft)', fontSize: 12, cursor: 'pointer' }}>
                    🔄 Verificar
                  </button>
                  <button onClick={handleRemoveDomain} disabled={removing} style={{ padding: '6px 12px', borderRadius: 8, background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', color: '#f87171', fontSize: 12, cursor: 'pointer' }}>
                    {removing ? '...' : '🗑️ Quitar'}
                  </button>
                </div>
              </div>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>
                Ingresa tu dominio propio (sin https:// ni www):
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                />
                <button onClick={handleAddDomain} disabled={saving || !domain.trim()} style={{
          ...
          {(instructions || domainStatus === 'pending') && (
            <div style={{
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12, lineHeight: 1.6 }}>
                En el panel de tu proveedor de dominio, agrega estos registros:
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 8, padding: '10px 14px' }}>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>Para www.tudominio.com</div>
                  <div style={{ fontFamily: 'monospace', fontSize: 12, color: 'white' }}>
                    CNAME &nbsp; www &nbsp; cname.vercel-dns.com
                  </div>
                </div>
                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 8, padding: '10px 14px' }}>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>Para tudominio.com (sin www)</div>
                  <div style={{ fontFamily: 'monospace', fontSize: 12, color: 'white' }}>
                    A &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; @ &nbsp;&nbsp; 76.76.21.21
                  </div>
                </div>
              ...
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 10 }}>
                ⏱️ Los cambios de DNS pueden tardar hasta 48 horas en propagarse.
              </div>
            </div>
          )}
          ...
          {message && (
            <div style={{
              ...
              background: message.startsWith('✅') ? 'rgba(52,211,153,0.08)' : 'rgba(248,113,113,0.08)',
            ...
            <div style={{ background: 'rgba(13,13,26,0.9)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 20, padding: '24px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12, lineHeight: 1.5 }}>PNG o JPG. Recomendado: cuadrado 200×200px</div>
