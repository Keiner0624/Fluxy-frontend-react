      ...
      {data.map((item, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, height: '100%', justifyContent: 'flex-end' }}>
            ...
            <div style={{
              fontSize: 9, color: 'var(--text-muted, #4a4a6a)',
...
function LineChart({ data, color = 'var(--primary, #7c83fd)' }) {
  ...
  const points = data.map((d, i) => {
    ...
    const y = PAD + ((max - d.total) / (max - min || 1)) * (H - PAD * 2)
    return { x, y, ...d }
  })
...
function StatCard({ icon, label, value, color = 'var(--primary, #7c83fd)', sub }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, color: 'var(--text-muted, #4a4a6a)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 8, fontWeight: 600 }}>
        {label}
      </div>
      <div style={{
...
export default function MetricsPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const [m, s, tm, tt] = await Promise.all([
        metricsRes.json(), salesRes.json(), topMonthRes.json(), topTodayRes.json(),
      ])
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{ marginBottom: 32, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <div style={{ fontSize: 11, color: 'var(--primary, #7c83fd)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: 6 }}>
            Panel de vendedor
          </div>
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 28, fontWeight: 700, color: 'white' }}>
            Métricas
          </h1>
          <p style={{ fontSize: 13, color: 'var(--text-muted, #4a4a6a)', marginTop: 4 }}>
        ...
        <button onClick={loadAll} style={{
          ...
          borderRadius: 11, padding: '9px 16px', color: 'var(--text-soft, #9898b8)',
      ...
      {error && (
        <div style={{ background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 10, padding: '12px 16px', marginBottom: 20, fontSize: 13, color: '#f87171' }}>
          ⚠️ {error}
        </div>
      )}
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
            <StatCard
              ...
              value={`S/ ${(metrics?.totalSales || 0).toFixed(2)}`}
              color="#7c83fd"
              sub="Pedidos completados"
            />
            <StatCard
              icon="🛒" label="Total pedidos"
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted, #4a4a6a)' }}>Ventas diarias completadas (S/)</div>
              </div>
              {salesPerDay.length > 0 && (
                <div style={{
                  background: 'rgba(124,131,253,0.08)', border: '1px solid rgba(124,131,253,0.15)',
                  borderRadius: 50, padding: '5px 14px', fontSize: 12, color: 'var(--primary, #7c83fd)',
                }}>
                  {salesPerDay.length} días
                </div>
              )}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20, marginBottom: 20 }}>
            ...
            <div style={{ background: 'rgba(13,13,26,0.9)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 20, padding: '24px' }}>
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: 'white', marginBottom: 2 }}>🏆 Top productos del mes</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted, #4a4a6a)' }}>Por cantidad vendida</div>
              </div>
              <BarChart
                data={topMonth.map(p => ({ label: p.productName, value: p.quantitySold }))}
            ...
            <div style={{ background: 'rgba(13,13,26,0.9)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 20, padding: '24px' }}>
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: 'white', marginBottom: 2 }}>🔥 Top productos hoy</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted, #4a4a6a)' }}>Por cantidad vendida hoy</div>
              </div>
              <BarChart
                data={topToday.map(p => ({ label: p.productName, value: p.quantitySold }))}
                labelKey="label" valueKey="value"
                color="#f59e0b"
              />
            </div>
          ...
          {/* Tabla top productos */}
          <div style={{ background: 'rgba(13,13,26,0.9)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 20, padding: '24px' }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: 'white', marginBottom: 20 }}>📊 Ranking de productos del mes</div>

            {topMonth.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--text-muted, #4a4a6a)', fontSize: 13 }}>
                No hay ventas completadas este mes aún.
              </div>
            ) : (
                  ...
                  <div key={i} style={{
                    ...
                    <div style={{
                      ...
                      background: i === 0 ? 'linear-gradient(135deg, #f59e0b, #d97706)'
                        ...
                        : i === 2 ? 'rgba(180,120,80,0.15)'
                        : 'rgba(255,255,255,0.05)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: 14, fontWeight: 800,
                      color: i === 0 ? '#fff' : 'var(--text-muted, #4a4a6a)',
                    }}>
                      {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : i + 1}
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-muted, #4a4a6a)', marginTop: 2 }}>
                        {p.quantitySold} unidades vendidas
                      </div>
                    </div>
                    ...
                    <div style={{ flex: 1, height: 6, background: 'rgba(255,255,255,0.05)', borderRadius: 3, overflow: 'hidden', minWidth: 80 }}>
                      <div style={{
                        ...
                        width: `${(p.quantitySold / topMonth[0].quantitySold) * 100}%`,
                    ...
                    <div style={{
                      fontFamily: "'Fraunces', serif",
                      fontSize: 16, fontWeight: 700, color: 'var(--primary, #7c83fd)',
